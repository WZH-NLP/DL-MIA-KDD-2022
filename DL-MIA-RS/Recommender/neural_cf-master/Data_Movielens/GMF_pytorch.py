...
def train(model, criterion, optimizer, epoch, batch_size, use_cuda,
    ...
    for data in train_loader:
        ...
        # print(type(preds))
        # print(preds)
        # predictions = predictions + preds.squeeze(1)
...
def evaluate(model, test_loader, use_cuda, topK):
    ...
    with torch.no_grad():
        ...
        for data in test_loader:
            ...
            randidx = np.arange(2000)
            np.random.shuffle(randidx)
            items, preds = items[randidx], preds[randidx]

            map_item_score = dict( zip(items, preds) )
            ranklist = heapq.nlargest(topK*2, map_item_score, key=map_item_score.get)
            ranklist = ranklist[0: topK]

            len_topk.append(len(ranklist))
            aaa = aaa + ranklist
            ...
            hits.append(hr)
            ndcgs.append(ndcg)
    return (np.array(hits).mean(), np.array(ndcgs).mean()), aaa, len_topk

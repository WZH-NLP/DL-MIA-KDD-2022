...
fr_shadow = open(path+"/amazon/amazon_music_topk_shadow_popular_lfm", 'r') # recommend for shadow
fr_target = open(path+"/ml-1m/movielens_topk_target_popular_itembase", 'r') # recommend for target
...
if not os.path.exists(DataDir):
    os.mkdir(DataDir)
ShadowDataset = open("../AttackData/trainForClassifier_ALMI.txt", 'w')
TargetDataset = open("../AttackData/testForClassifier_ALMI.txt", 'w')
for i in range(num_shadow):

...
for key, value in interaction_Smember.items():
    ...
    if torch.any(torch.isnan(temp_vector)):
        print("interaction_Smember:key:{},value:{}".format(key, value))
...
for key, value in recommend_Smember.items():
    ...
    if length != 0:
        temp_vector = temp_vector / length
    if torch.any(torch.isnan(temp_vector)):
        print("recommend_Smem:key:{},value:{}".format(key, value))
        print("temp_vector:", temp_vector)
...
for key, value in recommend_Snonmem.items():
    ...
    if length != 0:
        temp_vector = temp_vector / length
    if torch.any(torch.isnan(temp_vector)):
        print("recommend_Snonmem:key:{},value:{}".format(key, value))
        print("temp_vector:", temp_vector)
...
# ------------Target----------------
fr_vector_target = open(Vpath + "/beauty_itemMatrix.txt", 'r')  # vector for target items
...
Tmember_rec = open(pathT + "/beauty_Tmember_recommendations", 'r')  # recommend for target
...
Tnonmem_rec = open(pathT + "/beauty_Tnonmem_recommendation", 'r')  # recommend for target
...
# read interactions
itm = open(pathT + "/beauty_Tmember_train", 'r')  # interactions for target_member
...
if not os.path.exists(DataDir):
    os.mkdir(DataDir)
ShadowDataset = open("../AttackData/trainForClassifier_ABAB.txt", 'w')
TargetDataset = open("../AttackData/testForClassifier_ABAB.txt", 'w')

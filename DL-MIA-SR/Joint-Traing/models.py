...
class base(nn.Module):
    def __init__(self, vocab_size, embed_dim, embed_init, experiment):
        ...
        self.raw_feature1 = os.path.join(self.expe.config.root_dir, self.expe.config.raw_feature1)
        self.raw_feature2 = os.path.join(self.expe.config.root_dir, self.expe.config.raw_feature2)
        ...
        self.lambda_s = 1.0/self.expe.config.shadow_size
        self.lambda_t = 1.0/self.expe.config.target_size
...
class jointModel(base):
    ...
    def forward(self, sent1, mask1, sent2, mask2, tgt1,
        ...
        if submodel == "vgvae":
            ...
            # method 1 batch_size ---MLP--- batch_size sigmoid/softmax/relu batch_size -----get alpha_p
            self.index += self.expe.config.batch_size
            ...
            # print("ps pt should be 1xbatch_size, and each element should be (0-1)")
            # print("ps:", ps)
            # print("pt:", pt)
            # ps, pt must be (0-1)
            # method 2 pair_size ---MLP--- pair_size sigmoid/softmax/relu pair_size  ---look_up batch_size get alpha_p
            # ps = self.ps
            # pt = self.pt
            ...
            # bias_target, bias_target1, bias_target2, label_target, --> neg_sent2, neg_mask2, ntgt2, ntgt_mask2,
            # target
            # debiased feature vector
            ...
            # # 1 记得要改 self.reweight()
            # self.index1 += self.expe.config.batch_size
            # ps = ps[0][self.index1:self.index1 + self.expe.config.batch_size]
            # pt = pt[0][self.index1:self.index1 + self.expe.config.batch_size]
            # print("ps pt shoule be 0-1")
            # print("ps:", ps)
            # print("pt:", pt)
            # alpha_ps = self.reweight(ps)
            # alpha_pt = self.reweight(pt)
            # print("alpha_ps:", alpha_ps)
            # print("alpha_pt:", alpha_pt)
            # alpha_p = torch.sum(alpha_pt+alpha_ps).item()/(alpha_pt.shape[1]*2)
            # 2 记得要改 self.reweight()
            alpha_ps = self.reweight(ps)
            alpha_pt = self.reweight(pt)
            self.index1 += self.expe.config.batch_size
            ...
            alpha_p = torch.sum(pt + ps).item() / (ps.shape[0] * 2)
            # print("alpha_p:", alpha_p)
            # print("loss_vgvae_mean:", loss_vgvae_mean)

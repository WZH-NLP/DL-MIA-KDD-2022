...
# pydevd_pycharm.settrace('172.25.173.61', port=3931, stdoutToServer=True, stderrToServer=True)
...
# parser.add_argument('--train_data', default='ml-1m_Tmember_train', type=str)
# parser.add_argument('--valid_data', default='ml-1m_Tmember_test', type=str)
parser.add_argument('--train_data', default='beauty_Tmember_train', type=str)
parser.add_argument('--valid_data', default='beauty_Tmember_test', type=str)
parser.add_argument("--is_eval", action='store_true')  # should be used during testing and eliminated during training
parser.add_argument('--load_model', default=None, type=str)
# parser.add_argument('--checkpoint_dir', type=str, default='ml-1m_checkpoint_T')
parser.add_argument('--checkpoint_dir', type=str, default='beauty_checkpoint_T')
...
def main():
    ...
    # train_data.itemmap.to_csv("data/ml-1m_Titemmap.txt", index=False)
    train_data.itemmap.to_csv("data/beauty_Titemmap.txt", index=False)
    ...
    if not args.is_eval:  # training
        ...
        # trainer class
        trainer = lib.Trainer_T(model, train_data=train_data, eval_data=valid_data, optim=optimizer,

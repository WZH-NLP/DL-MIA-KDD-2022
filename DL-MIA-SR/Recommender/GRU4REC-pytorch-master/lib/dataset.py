...
class Dataset(object):
    def __init__(self, path, sep=',', session_key='SessionID', item_key='ItemID', time_key='Time', n_sample=-1,
                 itemmap=None, itemstamp=None, time_sort=False):
        # Read csv
        ...
        # print("self.itemmap:", self.itemmap)
        # print("self.df:", self.df)
        # print("self.click_offsets:", self.click_offsets)
        # print("self.session_idx_arr:", self.session_idx_arr)
...
class DataLoader():
    ...
    def __iter__(self):
        ...
        iters_old = np.arange(self.batch_size)
        maxiter = iters.max()
        ...
        while not finished:
            ...
            for i in range(minlen - 1):
                if maxiter == len(click_offsets) - 2 and i == minlen-2:
                    mask = np.arange(len(iters))
                    iters_old = iters.copy()
                # Build inputs & targets
                idx_input = idx_target
                idx_target = df.item_idx.values[start + i + 1]
                input = torch.LongTensor(idx_input)
                target = torch.LongTensor(idx_target)
                yield input, target, mask, iters, iters_old
